...
router.post('/login', async (ctx) => {
  ...
  if (data[user]) {
    ...
    data[user] = {pass}
    datas.user = data
    fs.writeFile('./data/options.json', JSON.stringify(datas), function (err) {
      if (err) {
          throw err;
      }
    })
...
router.post('/setoption', async (ctx) => {
  ...
  try {
    fs.writeFile(__dirname  + '/data/options.json', JSON.stringify(datas), function (err) {
      if (err) {
          throw err;
      }
    })
...
router.post('/getoption', async (ctx) => {
    try {

        let datas = fs.readFileSync(__dirname  + '/data/options.json', 'utf-8');
        datas = JSON.parse(datas)
        ctx.body = {
            code: 10000,
            data: {
